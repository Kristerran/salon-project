...
export default class AdminOrdersTable extends Component {
    constructor(props){
      super(props);
      this.state={
        apiList: []
          ...
          <tbody>
          {
            apiList.map((x, index)=>
              <tr key={x._id}>
                ...
                <td>{x.ref}</td>
                <td>{x.createdAt}</td>
                <td>{x.order.map(item=> <div>{`x${item.quantity} ${item.idItem}(${item.titleItem}[${item.selectedSize}] ${item.price}$) `}</div>)}</td>
                <td>{x.totalAmount+ ' $'}</td>
